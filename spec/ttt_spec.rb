...
describe 'TTT Service' do 
  ...
  it 'follows minimax path' do
    ...
    params[:cells] = convert_array_to_minimax_cells(['O', nil, nil, 
                                                     nil, 'O', nil, 
                                                     'X', nil, nil])
  ...
  it 'follows non-minimax path' do    
    ...
    params[:cells] = convert_array_to_regular_cells([nil, nil, nil, 
                                                     nil, 'X', nil, 
                                                     nil, nil, nil])

    next_cells = ttt.start_turn(params)
    expect(next_cells[0].value).to eq 'O' 
  end
  ...
  it 'blocks row correctly when minimax ai goes first' do
    ...
    params[:cells] = convert_array_to_minimax_cells(['X', 'X', 'O', 
                                                     nil, 'O', nil, 
                                                     nil, nil, nil])
    new_game_state = ttt.start_turn(params)
    next_cells = new_game_state.cells   
